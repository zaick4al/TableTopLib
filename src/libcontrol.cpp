...
template<typename T>
std::shared_ptr<T> LibControl::createEntity()
{
    auto currentIds = m_objects.keys();
    long long id = std::max_element(currentIds.begin(), currentIds.end(), [ & ](auto first, auto second) {
        return first < second;
    });
    return createObject<T>(id + 1);
}
